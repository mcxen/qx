...
- Top Bar 右上角在操作区与历史记录间切换；历史记录指最近选择或预览过的文件，不是重命名、压缩等操作日志。按最近查看时间倒序去重保留 5 条，显示名称、父目录与查看时间并支持清空；旧版操作历史不迁入。历史视图按 Esc 先返回操作区。
...
- 宏录制的指针采样不得压缩真实等待时间；尾部裁剪后补齐未释放的输入，不能把拖拽指针移回起点。
  暂停态仍可停止回放；回放延迟使用所选值。宏名称输入的 Enter/Esc 在 IME 合成期间归输入法。
...
- macOS 截图圈选与标注阶段的 picker/shade 必须直接操作其原生 `NSWindow`，使用
  `CGWindowLevelForKey(kCGPopUpMenuWindowLevelKey) - 1` 提升到菜单栏、状态项与 Dock 之上，
  同时低于系统输入法候选弹窗；不得使用会遮住候选词的 Screen Saver 层级。禁止再
  同时启用 Tauri 通用 `always_on_top` 覆盖该原生层级。窗口按显示器完整物理边界覆盖灰色蒙版
  并独占指针输入；底层桌面、菜单栏和 Dock 均不可点击。仅延迟
  倒计时和已经开始录制后的装饰边框允许按各自协议切换为鼠标穿透。
- 截图/录屏圈选采用 snapshot-first：在任何 picker/shade 映射前先冻结本次会话的全部显示器
  framebuffer，再把各显示器的不可变帧按 1:1 比例铺到最顶层窗口。圈选、预览、马赛克和
  最终 PNG 都必须读取同一份冻结像素，确认时不得重新采样实时桌面；会话结束后立即释放内存
  并异步删除临时整屏 PNG。由此菜单栏、状态项、Dock 和底层应用既不能被点击，也不会在
  标注期间继续变化。会话开始后热插入的新显示器留待下一次圈选，不得出现无冻结底图的透明层。
...
- 圈选操作提示默认显示在左下角；当选区与左下提示区域相交时自动避让到右上角。手动圈选
  覆盖整块显示器（允许边缘数个逻辑像素误差）或进入全屏选区时隐藏提示，避免遮挡选区。
...
- 截图文字工具启用后在选区内点击位置生成单字符宽编辑框；输入时按内容增长，选中态使用
  ...
  合成中的 Enter、Esc 与候选导航键必须留给输入法（包括 WebKit `keyCode=229` 的末次事件），
  合成结束后的 Enter 才结束编辑。文字增长到截图
...
- 箭头、矩形、画笔和马赛克拖拽必须由标注画布持有 Pointer Capture，以同步草稿保存最终落点，
  高频移动只按动画帧合并重绘；快速按下/松开或指针离开选区不得造成箭头偶发消失。文字新建
  点击不得被文档级“点击框外结束编辑”抢先清空。圈选提示默认靠近屏幕边缘，但必须实时避开
  当前选区；其他边缘均无完整空位时隐藏提示，不得覆盖待截图内容。
- 选区蓝框只能用不参与盒模型的 outline/独立绘制层，不能用 border 挤压内部标注画布；标注
  canvas 的 CSS 边界、指针坐标、Retina backing store 和最终导出必须映射到同一选区尺寸。
  前端 canvas 只负责交互预览；确认时只能把归一化的矩形、箭头、画笔、文字、序号与马赛克
  矢量协议交给 Rust 单一合成器。合成器先在冻结裁切图上执行隐私马赛克，再按 picker 会话的
  源图像像素比例绘制所有矢量；禁止导出浏览器透明 PNG 后缩放贴回成品。
...
Workbench 管理表单直接对齐详情内容边缘，参数组以分隔线划分，不堆叠嵌套卡片。
标签列统一 112px；表单自身宽度不超过 360px 时改为上下排列。输入与动作使用标准控件
尺寸，Select 仅由触发器绘制边框；按钮只使用共享 Button 的 variant/size，不在 Workbench
样式中覆盖高度、内边距、字号或颜色。重复记录可声明 `group.layout: "columns"`，宽栏使用
两列标签上置布局，窄栏回落为单列；组内 Select 按内容适宽并左对齐，不随单列铺满。
新增表单仅保留
必要字段与准确的保存提示，不重复展示空参数摘要、请求预览或与草稿保存行为冲突的说明。
...
- Launcher 结果单击只更新选择，双击或 Enter 执行主动作。文件交给系统默认应用打开；文件夹交给平台文件管理器打开（macOS Finder、Windows 文件资源管理器），不得走仅允许应用包或可执行文件的 `open_app` 端口。从 Qx 启动的应用应出现在 **Qx 当前所在的物理显示器** 上：已在该屏则只激活；在另一块屏则把可见窗口平移到该屏工作区。单屏、全屏窗口或缺少辅助功能时仍只负责启动。虚拟桌面 / Space 不在此范围。打开应用或文件后隐藏 Launcher 必须走 `floating_hide`，不得把焦点还回召唤前的应用。
- Launcher 每次修改查询或搜索范围后，默认选中可见结果中匹配度最高的条目；置顶分组顺序不决定默认选择，无关置顶项和分组标题不自动获选。同匹配等级先比较条目自身名称，再比较类型和使用频率。异步结果到达时可更新默认选择，但用户已用键盘或点击选择后，保持该条目直到查询改变或条目消失。
...
- Cards 的 `editor.format="markdown"` 按需启用宿主共享 Tiptap 与源码切换；未声明仍为纯文本。仅当前编辑卡片创建编辑器实例，工具栏保持紧凑并使用 Qx 控件/主题变量。进入编辑和切换模式不得标脏或改写原文；无法可靠转换的内容留在源码模式。编辑期间保持卡片阅读锚点，不让选中滚动与重新测量竞争；源码、中文 IME、原生选择/撤销以及脏草稿 Esc 仍走同一会话。
- Cards 使用真正的 Masonry 瀑布流：按源顺序逐项放入当前最短列，同高时从左到右；不按内容高度重排源数组，也不以最高卡片撑出等高整行。卡片按内容自然增高，短卡下方继续接下一张，宿主按容器宽度调整 1–3 列。测量、图片加载和原位编辑引起的高度变化必须更新后续位置而不重置阅读位置；只挂载视口及小幅 overscan，选中和编辑项保留挂载。
- 单击只选择；Enter 仍进入阅读；声明可编辑能力的卡片允许双击正文就地编辑，链接、图片和其他控件不得误触发编辑。卡片浏览默认占满主内容区，详情由用户按需打开。
...
- 资讯/社区阅读型 Workbench（含 `detail.body`、`detail.content[]` 或 `detail.replies`）把顶层
  `detail.fields[]` 统一投影到标题下的副标题元数据区，并按可用宽度自然换行；副标题中已经
  存在的同值作者、时间等不重复显示，正文下方不再渲染第二张属性表。管理型 Detail 的顶层
  `fields[]` 与 `sections[].fields[]` 继续使用结构化字段表，不能因同一端口而丢失分组语义。
...
- 左侧历史列表，右侧预览和信息。
- 列表、预览、信息区独立滚动。
- 图片列表只加载当前虚拟视口的最长边 320px 派生缩略图，右侧只加载最长边 1600px
  派生预览；两者进入 `clipboard-previews` 可重建缓存并通过 Tauri asset URL 呈现。禁止把
  原图 `Vec<u8>` 序列化成 JSON 数字数组、Data URL 或前端 Blob；原图只用于明确的复制、压缩、
  OCR 源语义和系统打开动作。派生缓存按源路径、字节数、修改时间和尺寸失效，最多 256 MiB /
  512 文件；单个源预览上限 128 MiB / 3200 万像素，超限显示可恢复失败态而不得拖死 Shell。
- 图片派生解码与剪贴板自动 OCR 共用同一个有界后台解码边界。打开 Clipboard 只并发读取热历史，
  不得在首屏关键路径再次读取/编码当前系统位图；实时内容由原生剪贴板监听器写库并发布更新。
...
- macOS 静态截图必须在圈选层出现前从完整显示器 framebuffer 取样，选区允许覆盖菜单栏、应用窗口标题栏与 Dock；不能以“工作区”或仅普通应用窗口列表代替整屏。活动 picker 与外屏 shade 都显示对应冻结帧并叠加灰色蒙版；最终截图直接裁切这份帧，确保用户标注的画面与成品逐像素一致。
- 显示器枚举、稳定 ID、内置/外接/主屏判断、鼠标所在屏幕和跨后端映射属于 Qx 系统级能力；截图、窗口管理、浮窗与热插拔监听必须消费同一服务，不得在模块内各自判断。圈选打开时先立即检测一次，随后以约 40ms 间隔检测鼠标所在显示器并随跨屏移动；一旦开始框选、已有选区或进入确认流程就停止跟随，避免编辑状态被迁移。圈选层按会话启动时的显示器拓扑为每个显示器创建显示冻结帧、拦截指针的半透明遮罩，只有鼠标所在显示器保留完整交互层；点击其他显示器遮罩只允许把圈选会话切换到该显示器，不得穿透并激活桌面应用。
...
- 选区建立后显示双态工具条，第一个图标在截图 / 录屏之间切换，每个图标必须提供本地化 Tooltip（用途、快捷键或禁用原因）。截图态依次提供区域、全屏、矩形、箭头、画笔、文字、序号、马赛克、撤销/重做、颜色、选项、✓ 确认和 × 取消；录屏态只提供区域录制、全屏录制、选项、✓ 确认和 × 取消，不提供任何标注或马赛克工具。区域与全屏按钮必须可往返并恢复最近区域选区；截图已有标注时切到录屏必须确认并清空标注及撤销栈，所有标注只进入 PNG。
...
- 思考耗时从首个真实 reasoning / tool 事件起按墙钟实时更新，完成时定格同一口径；折叠标题
  使用 `mm:ss` / `hh:mm:ss` 纵向翻页数字，减少动态偏好下只更新文本，不运行翻页和 shimmer。
- 文件能力优先使用宿主 `file_info / list_directory / glob_files / read_file / write_file / edit_file`
  端口；文本读取与遍历有界，既有文件必须先读并携带 revision 才可写入，精确编辑只允许唯一
  匹配。文件写入与编辑继续经过 dangerous-tools 确认，不能借由基础工具绕开 safety gate。
...
- AI Agent → Agent 与模型先展示聊天/默认模型，再展示紧凑供应商列表；工具总开关、原生
  Tool Calling 与迭代上限归入工具与安全。供应商新增/编辑使用独立 Dialog，内置模板只编辑
  API Key，不在列表上方插入表单。模型列表按所在栏宽响应，有界滚动；展开行保持线性分隔，
  不叠加背景卡片。设置导航默认 220px，仍保留宿主分栏拖拽与用户保存宽度。
  模型能力只在选择器标签中展示一次，
