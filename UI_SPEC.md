...
- Workbench 右侧图片详情必须由宿主适配横图、竖图、超宽图和窄内容区：详情图片默认按原始比例 contain，可声明 landscape/square/portrait 固定舞台；加载失败显示局部错误，点击可缩放图片打开宿主 Dialog，Esc 先关闭 Dialog。插件 iframe 不得注入宿主类名 CSS 或自建 lightbox 来修右栏。
