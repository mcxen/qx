...
- 截图文字工具点击图标后直接在选区中央生成单字符宽编辑框；输入时按内容增长，选中态使用
  虚线框、四角蓝色缩放点与右上角删除按钮。文字框可自由拖动，四角等比缩放必须同步字号；
  上下左右四条边均提供拖拽命中区：左右边改宽并重新排版，上下边改高但不得小于完整文字
  所需高度；角点纯横向拖动只改变框宽，斜向拖动才等比缩放文本框与字号。文字增长到截图
...
- Workbench 右侧图片详情必须由宿主适配横图、竖图、超宽图和窄内容区：单图使用
  ...
  Cmd/Ctrl+滚轮及 `+` / `-` / `0` 键盘操作。底部缩放控件不得使用成组大工具条；
...
`#楼号 / 作者 / 可选点赞数 / 楼主标记 / 时间 / 正文` 排列，点赞数紧跟作者右侧且不重复出现在正文；内置 V2EX 与插件 Workbench 共用同一
组件和样式。回复中的包内行内图片按约 1.45em 紧凑显示、随文字基线对齐，不进入全尺寸媒体预览；资源不可用时保留可读替代文本。全尺寸预览的左右边缘提供固定感应区：鼠标接近对应边缘或键盘聚焦时才
