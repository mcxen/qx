...
- 最小窗口：`480 x 360`
- 主壳圆角：`8px` 以内
- 控件圆角：默认采用 shadcn/Radix 源码组件的 `--radius` scale，并由 `--qx-control-radius` 统一约束；紧凑工具控件保持约 `6px`
...
设置页、模块工具栏和产品 UI 中的选择、开关、分段、弹层与滑动控件必须使用 Qx 的 shadcn/Radix 源码组件系统，避免出现平台原生外观不一致的问题。统一业务出口为 `src/components/ui.tsx`，底层 shadcn 源码位于 `src/components/shadcn/`。新增通用控件应先落在 shadcn 源码层或 Qx 门面层，而不是在模块内临时实现第二套。
...
- `Slider`：使用 shadcn/Radix Slider，不引入原生 range。必须支持 pointer drag、click-to-set、`ArrowLeft/Right/Up/Down`、`Home`、`End`，并提供 Radix slider 的 ARIA 语义。
- `Select`：使用 shadcn/Radix Select，不使用原生 `<select>`。必须支持键盘选择、`Esc` 关闭、blur/click outside 关闭。`value: "---divider---"` 是保留分隔线约定，只渲染分隔线且不可选。
- `Toggle`：使用 shadcn/Radix Switch 或 button 类控件表达开关状态，视觉状态使用 CSS 变量。
- `SegmentedControl`：使用 shadcn/Radix ToggleGroup 表达互斥选项，选中态不得依赖原生 radio 外观。
- `Modal` / 弹层 / Action Menu：优先使用 shadcn/Radix Dialog、Popover、DropdownMenu，弹层表面必须消费 Qx 半透明 surface 变量。

视觉规则：

- 控件圆角遵循 shadcn `--radius` scale，并映射到 `var(--qx-control-radius)`；同一屏内不得出现旧控件圆角和 shadcn 圆角混杂。
