...
- Workbench List / Gallery 的内容轨是稳定宿主表面：空数据或少量数据时仍保留完整列表栏、分隔边界与 Gallery 画布；空态必须跨满所属区域并垂直居中，不能缩成首个 grid cell 或随 item 数量塌缩。
...
- Appearance → Desktop Floating Island 是 Qx 级总开关，不归番茄钟或任一插件所有。
  浮窗默认位于**主显示器工作区右上角**，只显示 host task 或获 `island` 权限的插件
  结构化 slots；背景透明度、主题、圆角与 docked `QxIslandSurface` 使用同一组 token。
  多个普通 `location` session 按用户设置的时间间隔公平轮播；`task > error > toast`
  会立即抢占，结束后恢复轮播。插件不能提供自定义 chrome、窗口坐标、轮播周期或置顶
  策略。主窗隐藏显示与 always-on-top 均由用户设置决定，同一 session 默认不在
  docked / floating 双显。
- 悬浮 Surface 最右侧固定提供宿主级“缩小 / 展开”和“打开 Qx”图标按钮。展开宽度
...
同优先级中的长期模块/插件位置态不以“最后一次更新”永久霸占：`location` 进入统一
轮播队列；进度、倒计时心跳和文字更新不得重置轮播或抬高排序。任务、错误、通知按
上述优先级抢占，生产者不得用高频更新模拟重点事件。
...
- 捕获历史支持 **列表 / 图库** 两种持久化视图：两种视图都占满 Main Area，选择条目后在同一主内容区进入全宽预览，不渲染固定右侧详情面板。两种视图必须共享选择、预览、删除、Shell 键盘导航和 Actions，不得维护两份历史状态。
- 捕获历史默认使用 Gallery：浏览态让缩略图网格占满 Main Area，选择卡片后进入全宽独立预览，Esc 返回 Gallery；List 仍作为用户主动选择的紧凑模式保留。
...
- **禁止**把 `Alt+Space` / `Option+Space`（默认窗口召唤）或 `Cmd+Space` / `Ctrl+Space`（系统 Spotlight 等）绑成模块 Action；Shell 匹配层必须放行这些宿主级组合键，不得 `preventDefault`。
