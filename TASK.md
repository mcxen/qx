...
- 修正 AI Agent 模型配置页归组：供应商卡不再混入记忆管理，默认模型紧随供应商显示；重复的模型能力标签移除，记忆管理归入“工具与安全”并补齐中文、有界滚动和窄窗布局。
- 修正折叠思考继承旧卡片背景及标题双层 shimmer：常态透明、动画仅裁切在标题字形内，减少动态偏好下停止 shimmer。
- 收敛对话正文元信息：消息日期改用 resolved locale 短格式，助手左对齐、用户右对齐，降低字号/对比度并保持 hover/focus 操作出现时不位移。
- 工具折叠摘要不再把 JSON 的末尾 `}` / `]` 或空 `{}` 当成活动文本；无语义字段时只保留工具状态，完整结果仍可展开查看。
...
- 固定插件开发提交 e02ed03；此次宿主 tag 不发布插件市场包。
- 本地检查通过；tag 流水线与安装包状态单独确认，外接设备矩阵仍待验。

### Refactor — 显示器亮度与原生能力消融（2026-09-08）

- 亮度编排从 display/capture 拆出；原生 Apple 外接屏、Intel I2C、Apple Silicon 排他匹配、
  Windows VCP/高层兼容及独立软件调光已实现。插件 2.0.0 迁到 Workbench，要求宿主 0.6.109。
- 旧表单/新 slider、DDC 解析与模糊匹配、分后端/组合、写入合并/错误/销毁回归通过。
  浏览器 24 组真实控件布局通过，本机内置屏和软件调光读写/恢复通过；外接 DDC、Intel
  真机、Windows 编译及安装态待验，不上架未经设备主路径验证的插件。
- 全部 324 注册命令的盘点已接入门禁；修复 URL 插件落盘/解压的阻塞边界和共享 Slider
  手柄缺失辅助功能名称。基线 v0.6.108 全量门禁 + 231 Rust tests 通过。
- 最终组合门禁、前端 build、232 Rust tests 通过；安装线程修复独立消融通过。
  亮度界面使用“硬件 / 软件 / 恢复”，移除重复说明和原始值展示。
- 细项、剩余边界与超过发布阈值后的真实设备门禁见
  [原生能力消融记录](docs/native-capability-validation.md) 和 [亮度服务](docs/display-brightness.md)。

### Feature — 启动应用跟当前物理显示器（2026-09-07）

- `open_app` 与全局应用快捷键在多显示器下把目标应用的可见窗口放到 Qx 所在物理屏工作区；单屏、全屏窗口或缺少辅助功能时仍只启动。不处理虚拟桌面。
...
**状态**：实现、专项门禁、全量门禁、前端生产构建与 Rust 检查完成；待安装版交互复核。

- 设置页模型只作为新会话默认值；会话右侧模型只决定该会话后续 turn。
- 每条助手回复、重新生成候选和活动流保存实际 provider/model，历史消息不再跟随当前选择统一改名。
- 旧会话首次加载时以现有会话模型冻结缺失快照；Provider 请求剥离这些本地元数据。
- `node scripts/check-qx-ai-agent.mjs`、`npm run check`、`npm run build`、`cargo fmt --check`、定向 Rust 测试与 `cargo check` 通过。
