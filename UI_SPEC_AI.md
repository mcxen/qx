...
Settings → AI Agent 的记忆管理使用紧凑工具栏与列表，提供四类记忆选择和“压缩记忆”。
压缩运行时显示不确定进度并禁用重复写入；完成显示 active 正文字数变化，无收益时如实提示。
原文保留并标记为已归档，错误走现有 Island；不把归档条数增加误报为压缩失败。
记忆范围独立于分类：会话 Context 与窄窗 Actions 均提供项目名称输入，留空表示全局，
修改仅影响后续 turn。管理页按范围筛选并搜索，来源浮层可查看保留原文或打开来源会话；
删除需确认。自动整理错误留在单行 Island 并提供重试，管理页保留待整理重试入口；
不在正文、列表或输入框下新增错误条，也不把兼容镜像字数当作实际索引 token 用量。
