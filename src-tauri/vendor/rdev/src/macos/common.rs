...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // Do not translate key codes through TIS/TSM from the CGEvent tap
        // callback. On recent macOS releases those APIs assert that they run
        // on the main dispatch queue, while this callback runs on rdev's
        // input-event thread. Qx only needs the physical `Key` value; leaving
        // `Event::name` empty preserves that signal without the crash path.
