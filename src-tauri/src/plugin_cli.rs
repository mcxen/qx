...
//! - `plugin_system_*`: platform env + open/reveal paths (permission `system`)
...
// ---------------------------------------------------------------------------
// System capabilities (platform env + open/reveal)
// ---------------------------------------------------------------------------
...
pub fn plugin_system_env() -> PluginSystemEnv {
    ...
    let home_dir = crate::paths::home_dir().display().to_string();
...
fn validate_user_path(path: &str) -> Result<PathBuf, String> {
    ...
    if raw == "~" {
        p = crate::paths::home_dir();
    } else if let Some(rest) = raw.strip_prefix("~/").or_else(|| raw.strip_prefix("~\\")) {
        p = crate::paths::home_dir().join(rest);
    }
    Ok(p)
}

/// Open a file/dir/URL with the OS default handler.
#[tauri::command]
    ...
    tauri::async_runtime::spawn_blocking(move || {
        #[cfg(target_os = "macos")]
        {
            let status = Command::new("open")
                .arg(&path)
                .status()
                .map_err(|e| format!("open {display}: {e}"))?;
            if !status.success() {
                return Err(format!("open {display} failed with {status}"));
            }
            Ok(())
        }
        #[cfg(target_os = "windows")]
        ...
        #[cfg(target_os = "linux")]
        {
            let status = Command::new("xdg-open")
                .arg(&path)
                .status()
                .map_err(|e| format!("xdg-open {display}: {e}"))?;
            if !status.success() {
                return Err(format!("xdg-open {display} failed with {status}"));
            }
            Ok(())
        }
...
/// Reveal a path in Finder / Explorer / file manager.
#[tauri::command]
pub async fn plugin_system_reveal_path(path: String) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        #[cfg(target_os = "macos")]
        {
            ...
            if !status.success() {
                return Err(format!("reveal {display} failed with {status}"));
            }
            Ok(())
        }
        #[cfg(target_os = "windows")]
        {
            let arg = format!("/select,{}", display);
