...
4. **可脱离主窗口**（通用岛）：label `island` 轻量 webview；默认不抢焦点；可点击；主窗 hide 后 sticky task 可提升（设置控制）。
...
- v1 **不**做多枚 docked 岛；浮窗池 1 个 active `island` 窗。
...
| ≤680px | docked island `display: none`；**task/error** 必须在模块主 UI 有等价态；若 `island_float_enabled` 且 session 允许 float，可 promote 浮窗 |
...
```text
┌────────────────────────────────────────────────────────────┐
│ [identity] primary … secondary     [activity?] [action?]   │  ← 一行，align center
│ ▓▓▓▓▓▓▓░░░░ progress 2px overlay along bottom edge         │  ← absolute bottom inset
└────────────────────────────────────────────────────────────┘
height: 34px; overflow: hidden;
trailing pack (right):  [activity][action]  — action always rightmost when present
```

**Trailing 锁定规则（PR1 唯一解释）**：

- 右侧为 **inline pack**：`activity`（若有）在左，`action`（若有）**永远最右**。
...
| + action | trailing pack **最右** button，不换行 |
| + activity + action | `[activity][action]`，action 最右 |
| empty | `visibility: hidden` 保布局（docked） |

`prefers-reduced-motion: reduce` 关闭 bounce/wave。
...
- 若 `island_float_enabled` 且该 session 满足 §7.3 float 资格（`placement` + `sticky` + `priority === "task"`），**自动 `requestFloat`（main-visible dual 规则见 §7.3）** 或在 exception 挂载时 promote 到浮窗，避免长任务“静默消失”。
- exception 卸载后：取消抑制；若 `prefer_docked_when_main_visible`，浮窗可收回，docked 恢复 store winner。
...
- `island` → upsert `module.${routeKey}.shell` + bind onAction；unmount 仅 dismiss 该 shim id；
...
#### 5.2 插件 `islandHost.show` 策略（v1）— **唯一权威**
...
Always-on-top 浮窗 **不对插件开放** v1。与 §4.3 映射表冲突时 **以本节为准**。
...
| resizable | false |
| inner_size | 逻辑 **400×34**（与 CSS token 一致；DPI 由 Tauri scale） |
...
#### 7.3 几何与多显示器（Issue 4 — 锁定 v1 默认）

**v1 默认算法**（对齐 `position_controls` 思路，位置改底中）：

1. 解析 **anchor monitor**：  
   - 若 main 可见：`main.current_monitor()`；  
   - 若 main 已 hidden：使用 **lastMainMonitorId**（每次 main show/move 时写入 Rust `Mutex`）；若无则 **cursor monitor**，再无则 primary。  
2. 取 monitor `work_area()` 与 `scale_factor()`。  
3. 逻辑尺寸 W=400、H=34 → 物理宽高 `round(W * scale)` 等。  
4. 默认位置：**work area 水平居中、底边向上 margin 24px**（物理坐标）。  
5. Phase 2：拖拽后写入 settings `appearance.island_float_x/y` + monitor id；有记忆则覆盖默认。  
6. 与 `recording-controls` 共存：录制控件保持 **其现有右上**；island 底中。重叠时录制优先（z：后 show 在上；录制 show 时不强制 hide island，验收不遮挡 Stop）。

**Promote 触发（main 隐藏）**（`placement` 含 float 语义且 settings 允许）：

挂到 **所有** main hide 路径：

- `floating_hide`  
- `floating_hide_restore_focus`  
- blur-hide  
- close-to-background  

条件：`island_float_enabled &&`  
`shouldFloatOnMainHidden(session)` =  
`placement ∈ {floating, docked-or-float} && sticky && priority === "task"`  
（**不是** toast；插件永不满足）。

若 hide 时 docked 被 exception 抑制，仍对 store 中 **合格 sticky task** 做同样 promote（§3.6）。

Accessory 政策下 secondary webview **保持可见**（验收：hide main 后 island 仍显示）。

Main 再 show 且 `island_prefer_docked_when_main_visible`：hide `island` 窗，session 回 docked（若无 exception 抑制）。

##### `requestFloat` / `placement: "floating"` 在 main **可见**时（v1 锁定）

**策略 B — float 主要为 hide-main 与 exception 遮挡服务；禁止 main 可见时的双显。**

| API / 字段 | v1 行为 |
|---|---|
| `placement: "docked"` | 仅 docked；`requestFloat` → no-op |
| `placement: "docked-or-float"` | main 可见且无 exception 抑制 → **只 docked**；main 隐藏或 §3.6 exception 遮挡 → 可显示 float |
| `placement: "floating"` | v1 **降级为 `docked-or-float` 同语义**（dev warn 一次）；不单独实现「仅浮窗、永远不占 docked」 |
| `requestFloat(id)` | 允许调用方：模块 host / §3.6 exception 自动路径。条件：`island_float_enabled` && session 存在 && `placement ∈ {floating, docked-or-float}` && `priority === "task"` && `sticky === true`。若 **main 可见且无 exception 抑制** → **no-op**（返回 false）。若 main 隐藏 **或** exception 抑制 docked → show float。 |
| 双显 | **v1 禁止** 同一 session 同时 docked + float 可见 |

谁可调用：模块代码与 DockSlot 策略；**用户手势**可后续接设置，非 v1。插件 **不可** `requestFloat`。
...
|---|---|
| 首屏不 await | 占位 + cache |
| 生命周期独立 | sticky session + float promote |
...
| `ScreenRecorder` `customIsland={<RecordingTransport/>}` | **v1 例外（permanent for PR1–4）** | 保留 main `customIsland` + 独立 `recording-controls`；**DockSlot Option 1**：exception 抑制 store docked；合格 sticky task 可 float（§3.6 / §7.3）；**不**并入 `island` label |
...
| Key | 默认 | 说明 |
|---|---|---|
| `island_float_enabled` | **`false`**（PR4 dogfood；稳定后可改 true） | 总开关 |
| `island_float_when_main_hidden` | `true` **仅当** enabled；且代码侧再过滤 `sticky&&task` | 非 toast 自动浮起 |
| `island_float_always_on_top` | `true` | |
| `island_prefer_docked_when_main_visible` | `true` | |
| `island_float_x` / `island_float_y` / `island_float_monitor` | unset | phase 2 拖拽 |
...
| Command | 谁调用 |
|---|---|
| `island_window_ensure/show/hide/set_position/set_size/set_always_on_top` | **仅 host**（main session sync） |
| `island_window_get_snapshot` | float cold start |
| `island_sessions_publish` | main → Rust mirror |

### 插件 RPC

- method + permission **`island`** default deny；slots + toast 级约束见 §5.2。
...
- appearance 布尔/坐标字段。  
- Rust `lastMainMonitor` + optional snapshot `Mutex`。
...
| 插件 always-on-top 钓鱼 | 高 | v1 禁止插件 float；priority **仅 toast**；TTL≤8s；长度/频率限制；permission default deny |
...
1. ~~浮窗默认位置~~ → **已锁定** §7.3 底中 + lastMainMonitor。  
2. ~~dual~~ → **v1 不做**；类型已删除。  
...
6. **插件 v1：slots + priority 仅 toast + docked-only + TTL≤8s + permission default deny**  
   - activity/success/失败均 toast（tone 区分）；**禁止** plugin task/error/sticky/float；120s 安全网仅模块 task。
...
13. **Float 默认几何锁定：lastMainMonitor work area 底中 margin 24px**  
    - hide 全路径 promote 仅 sticky task（且 flag on）。

14. **设置：appearance 切片；`island_float_enabled` 默认 false**  
    - 与 dogfood rollout 一致。

15. **Exception customIsland 抑制 store docked（Option 1）；session 保留；合格 sticky task 可 float**  
    - 避免录制 HUD 下全局任务静默丢失或双绘。

16. **main 可见时禁止 docked+float 双显；`requestFloat` 仅在 hide-main 或 exception 遮挡时生效**  
    - `placement: "floating"` v1 降级为 docked-or-float 语义。
...
- **测试必须**：resolveDockedWinner、CAS stale、TTL、shim unmount 范围、`resolveDockedRenderMode(exception)`、plugin reject task priority、requestFloat no-op when main visible。
...
### PR4 — Floating webview + hide-path promote + intent bridge
...
- **Description**：screencap 旗标；§7.3 几何；sticky task promote；`island:intent`→registry；float **slots-only**；flag 默认 **false**。  
- **验收**：多显示器 lastMainMonitor；main hide 全路径；Win/mac 不抢焦点；与 recording-controls 共存。
...
*End of design document (rev 3.1 — open questions resolved).*
