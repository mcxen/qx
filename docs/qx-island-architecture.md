...
`plugin-display` 仍是 slots-only，primary ≤ 80、secondary ≤ 120；宿主强制
`priority: "location"`、`placement: "docked-or-float"`、`sticky: true`。插件不能
指定 component、窗口位置、置顶策略或任务优先级，只能提供真实 0–100 progress、
可选 TTL，以及一个映射到自身 manifest command 的 action。是否启用独立浮窗、
主窗隐藏时是否显示、是否置顶，全部由用户在 Settings → Appearance 决定。
...
`placement ∈ {floating, docked-or-float} && sticky &&`
`(priority === "task" || source === "plugin-display")`。普通 plugin toast 永不满足。
...
谁可调用：模块代码与 DockSlot 策略。插件不可调用 `requestFloat`；插件 display
是否浮出由宿主设置和主窗可见性统一决定。
