...
| **Author** | — |
| **Date** | 2026-07-15 |
| **Status** | Implemented (docked + desktop float rotation; Qx-owned policy) · rev 3.2 |
...
1. anchor 固定为 **primary monitor**，让悬浮岛拥有稳定、可预期的桌面位置。
2. 取 monitor `work_area()` 与 `scale_factor()`，避开菜单栏 / 任务栏。
3. 逻辑尺寸 W=400、H=34 → 物理宽高 `round(W * scale)`。
4. 默认位置：**工作区右上角**，top 16 / right 20 logical px。
5. Phase 2 可加入拖拽记忆；未配置坐标时永远回落到上述主屏右上锚点。
6. `recording-controls` 继续按捕获区域或工作区底中定位，两类浮窗自然分区。
...
条件：`island_float_enabled && placement ∈ {floating, docked-or-float}`，候选包括
host task/error/toast 与获准的 `plugin-display` / module location。`home` 与纯 docked
session 不进入桌面浮窗。
...
- Float chrome 最右侧由宿主固定渲染两个控制：`compact/expand` 调用
  `island_window_set_compact` 同步收缩真实窗口；`open-main` 经 `island:intent` 回到
  main bridge，再调用 `floating_show`。插件不能覆盖、隐藏或绑定这两个按钮。
...
| `island_float_rotate_secs` | `8` | 普通 module/plugin `location` session 轮播；任务/错误/通知抢占 |
...
| `island_float_x` / `island_float_y` / `island_float_monitor` | unset | phase 2 拖拽；unset 时主屏工作区右上，top 16 / right 20 logical px |
...
- appearance 布尔/坐标字段。  
- Rust session snapshot `Mutex`；拖拽坐标记忆为后续兼容字段。
...
- **Description**：screencap 旗标；§7.3 主屏右上几何；多 session 轮播与事件抢占；`island:intent`→registry；float **slots-only**；flag 默认 **false**。
- **验收**：主屏工作区右上；main hide 全路径；Win/mac 不抢焦点；与 recording-controls 共存。
