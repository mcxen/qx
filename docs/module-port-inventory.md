...
| Actions 数据 / 右栏渲染 | **`QxShellAction[]` + `primaryActionId` + `QxActionList` / `QxActionSections`** | Workbench 发布 action descriptor；自定义 HTML panel 用 **`context.ui.mountActions`** | 稳定动作 ID 驱动 Bottom Bar 与 Enter；`primary:false` 的自定义 panel 动作只进入 Context / Actions 菜单。完整 Context 型页面只投影非 primary 业务动作，多个领域组统一交给 `QxActionSections` 跳过空组，并关闭重复 Actions 菜单。manifest 启动/后台命令及宿主 reload 不混入插件业务 Action；宿主仅以保留 `__qx:*` id 追加结构动作和通用能力：资讯详情自动提供离线 HTML 快照保存，序列化当前 `body/content/fields/sections/replies`，以 4 路有界并发把 HTTP(S)（带 Referer）和包内 asset 图片转成 Data URL 后写入 Downloads；内置 RSS 下载文章复用同一 `inlineRemoteImagesInHtml` 端口改写正文 HTML，任一图片失败则不落盘且不回传插件 handler；Shell 通过 `data-qx-list-index` + `navigation.onChange` 统一处理条目右键，模块不得复制菜单 |
...
|---------|------------|-----------|----------|------|------|
| **another-boring-piece** | ✅ | ✅ | **host Workbench List + media detail + Actions**；http + system wallpaper/download + 精确文件写入 + clipboard/open-url/notifications | 6 小时 catalog SWR + 20 槽本地壁纸环 + 200 条用户历史 | 今日作品/历史收敛到单面板；自动轮换默认关闭且按 30 分钟 heartbeat 检查用户间隔；保留原作者 `yevgen_glukhov` 与贡献者署名；无 Raycast runtime/shim、自绘 shell/CSS 或无界文件缓存 |
