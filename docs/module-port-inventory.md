...
| Actions 数据 / 右栏渲染 | **`QxShellAction[]` + `primaryActionId` + `QxActionList`** | Workbench 发布纯 action descriptor + primary id，宿主映射一次 | 稳定动作 ID 同时驱动 Bottom Bar、Enter、Cmd/Ctrl+K、右键 Context Popover；Shell 通过 `data-qx-list-index` + `navigation.onChange` 统一处理条目右键，模块不得复制菜单 |
