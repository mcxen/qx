...
| **qx-ai** | list / chat / settings | shell（含 chat/settings leave 父级） | list selection（会话） | `QxModuleSearch` + loading | AI store；默认启用 Agent / tools / Bash；文件名搜索复用完整 `search_files`；宿主动作复用 `system/pathActions`、原生文件剪贴板与文件 metadata | AI 可打开/定位路径、复制文本或真实文件，并把本地文件作为带操作按钮的对话附件发送给用户；可见副作用仅在用户请求时调用 |
