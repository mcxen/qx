...
|------|------|-------------|-------------|--------------|-------------|-------------|
| **clipboard** | 全屏面板（**核心**，eager import） | `useQxModuleShell` + stepBack | `useQxListSelection` + `@tanstack/react-virtual` 有界 DOM | `QxModuleSearch` | Rust clipboard DB；**打开端口** `openSession.prefetchClipboardOpen`（热窗口 history 并发；idle warm；SWR 先画 store）；**冷存储** `loadMoreClipboardHistory`（列表滚到底 / 键盘近底加载） | 快捷键打开：navigate 即预取热窗口；同帧调用合并为一次打开事务，当前剪贴板图片探测 10 秒节流；滚底再拉更早记录；缩略图只对视口项以 4 路有界并发读取；面板非 lazy |
