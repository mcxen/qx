...
| **qx-ai** | list / chat / settings | shell（含 chat/settings leave 父级） | list selection（会话） | `QxModuleSearch` + loading；生成中消息队列；`/` Skill 模糊选择 | AI store；默认启用 Agent / tools / Bash；文件名搜索复用完整 `search_files`；宿主动作复用 `system/pathActions`、原生文件剪贴板与文件 metadata；`~/.qx/skills` 由异步 Rust 端口枚举/读取 | AI 可打开/定位路径、复制文本或真实文件，并把本地文件作为带操作按钮的对话附件发送给用户；可见副作用仅在用户请求时调用；选中 Skill 只作用于下一条消息 |
