...
| Launcher Home / Tray 轻量数据 | **`src/plugin/surfaceProviders.ts` + Rust 领域服务** | `manifest.homeWidgets[]` 关联 Home；`manifest.surfaceProviders[]` 关联 Tray/Home | 宿主绘制、采样、写入、排序和节流；Provider 插件可 manifest-only 懒加载，不提供 DOM/CSS/私有轮询 |
