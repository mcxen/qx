...
| **macros** | 录制器 | shell + 独立 `MacroCaptureSession` stop port | — | — | macro store；原生 raw event 有界队列 | Esc、按钮、模块卸载、应用退出统一 stop；macOS event tap / Windows keyboard+mouse hook 不复用截图指针监听 |
