...
| Actions 数据 / 右栏渲染 | **`QxShellAction[]` + `primaryActionId` + `QxActionList`** | Workbench 发布纯 action descriptor + primary id，宿主映射一次 | 稳定动作 ID 同时驱动 Bottom Bar、Enter、Cmd/Ctrl+K、Context；Shell 自己生成 Actions 入口 |
