...
| **qx-ai** | workbench（左会话列表 + 右自绘聊天）/ settings | shell；settings leave → workbench；Esc leave → launcher | `useQxListSelection` + `useQxMasterDetail` + `QxResizableSplit` | 顶栏搜索过滤会话；`/` Skill；消息队列 | AI store；上次会话；**Capability 目录** `list_qx_capabilities` / `run_qx_capability` + `list_plugins` / `run_plugin_command`；Skill frontmatter `capabilities:` 绑定模块/插件；Module Action；`~/.qx/skills` 种子 skill | Skill 驱动执行 Qx 模块与插件；插件 `context.ai.actions.*` + launcher command 可扩展；Workbench 右栏自绘对话 |
