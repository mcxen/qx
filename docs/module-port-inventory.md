...
| **file-actions** | Finder / Explorer 选择列表 + 操作工作区 | `useQxModuleShell` | `useQxListSelection` + `QxResizableSplit` | — | 根级 Rust `file_manager` 服务；插件复用 `context.files.selection/performSelectionOperation` | 唤起 Qx 前捕获前台文件管理器 HWND / Finder；快照 revision 约束重命名、归拢、ZIP 压缩与安全解压；写操作在 blocking worker |
