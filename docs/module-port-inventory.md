...
| 列表选中 / 滚入视口 | **`useQxListSelection`** | 声明式 Workbench List/Gallery 由宿主处理；custom panel 自理 | DOM：`qx-list-row` + `is-active`；Workbench List/Gallery 统一按视口虚拟化并保留完整键盘索引，只挂载可见条目与小幅 overscan，远程缩略图解析上限 4 路并发；List 的 `item.image` 为行缩略图，`item.images[]` 为社区动态紧凑卡片，`detail.image(s)` 为自适应右侧媒体且可用 `mediaPlacement="after-body"` 跟随文章正文；详情图片集合按插件发布数量完整呈现，不人为截断；需要原位图文顺序的长文使用纯数据 `detail.content[]` text/image/asset-image 块，正文与 `detail.replies.items[].content[]` 共用 Workbench 行内内容协议；趋势/时间序列使用结构化 `detail.chart`，宿主以 Qx shadcn/Radix token 绘制折线图，插件不得注入 SVG/Canvas 或业务颜色；`points` 只允许真实源数据或持久化真实采样，少于两个点时不绘制曲线；Workbench 与内置 RSS 共用 `QxMediaViewer`，全尺寸预览统一横图按宽、竖图按高、超长截图按宽滚动，放大后支持拖拽平移；宿主预解码相邻图片并按最后访问时间保留 15 分钟/24 张（缓存是性能策略，不是集合上限）；预览左下角提供下载原图；`detail.replies` 由共享 `QxReplyList` 在底部显示 `#楼号`、作者、作者右侧点赞数、时间与 OP，正文可用纯文本或受限的 text/image/包内 asset-image 行内片段，RSS 的 V2EX 文章回复同样复用；宿主提供失败态/放大预览，`item/detail.status` 通过共用 activity 字段表达真实百分比或 completed/total/failed；社区内容的 indeterminate 正文、评论与图片加载统一投影到 Workbench `island.activity`，不在 `detail.status` / `detail.replies.status` 重复显示，内联 status 仅保留错误或确属内容区域的可量化进度；`detail.form` 为宿主渲染的 text/number/select/textarea/slider 受控表单，长文本上限 64 KiB；连续控件可用 `group.id` 归组，首个控件声明 `group.layout: "columns"` 得到两列/窄栏单列的自适应布局，组删除经 `group.action` 发布；`mountWorkbench()` controller 的 `updateItems` 按稳定 id 增量/批量合并并仍发布完整快照；浏览态全宽集合，激活带详情条目后由宿主挂载左集合 + 右详情；宿主乐观选择后通知插件；隐藏 Workbench iframe 的集合导航键转交宿主 Shell；详情打开后 region 键驱动当前集合或阅读区 |
...
| 内容瀑布流 | **`workbenchMasonry` + `workbenchKeyboard`** | Workbench `layout.kind="cards"` | 宿主按源顺序向最短列落位，按自然高度重新测量、保留阅读锚点和选中/编辑项；方向键消费完整集合的空间邻居，不使用等高行偏移；插件只发布内容与可选 editor |
| Markdown 原位编辑 | **`WorkbenchMarkdownEditor` + `workbenchMarkdown`** | Workbench `item.editor.format="markdown"` | 缺省纯文本；宿主按需加载 Tiptap、保留源码及草稿，插件仍收发字符串并负责版本/附件/写权限；不新增 RPC 或私有插件编辑器 |
...
| 已安装插件更新目录 | **`useMarketplaceCatalog` + `fetch_plugin_index` / `download_plugin` / `install_plugin`** | 无；插件不自检更新 | 后台只拉索引，对照本机版本后在 Installed 列表/详情就地升级；手动升级按插件 ID 串行排队，当前项不中断，单项失败不取消后续。`marketplace_update_compatible_plugins` 仍只由 `auto_update` 自动安装。选择规则与自动升级相同（平台、`min_app_version`、更高版本）。 |
...
| 模块搜索框 | **`QxModuleSearch`**（默认不自动聚焦；首要搜索页显式 `autoFocus`） | Workbench 由宿主渲染受控 query；custom panel 自绘 input | Workbench 宿主立即回画输入、140ms 防抖后通知 handler，清空立即发送且 tab/filter/销毁取消旧查询；handler 更新业务结果后重新发布；pointer 进入表单/详情后不得抢回搜索焦点；Launcher 搜索另见 SearchBar |
...
| 跨会话缓存 | localStorage / Rust 磁盘缓存 | **`context.storage.persist`** | Workbench 呈现快照由宿主自动 SWR、持久读取 single-flight 且同进程热命中；业务原始响应/可操作文件仍由领域缓存负责 |
...
| 灵动岛 | `island` prop / **`islandHost`** | **`context.island`** | 权限 `island`；真实进度可声明宿主受控 `progressStyle`（默认 `surface-fill`，另有 `icon-ring/island-ring/compact-line`），禁止插件注入视觉代码；`QxShell.islandKey` 必须稳定并由 Shell 绑定内置模块 `openTarget`；插件目标由 bridge 绑定；store 单写、DockSlot 单渲染；前台非粘性 location 高于后台粘性轮播；桌面浮窗只由用户从 Qx 手动浮出并可关闭；docked 双击展开宿主最近 5 个浏览图标，弹出/收回只走 `island/recents/recentMotion.ts`（已有 framer-motion），插件不得注入该切换器或 Motion |
...
|------|------|-------------|-------------|--------------|-------------|-------------|
| **clipboard** | 全屏面板（**核心**，eager import） | `useQxModuleShell` + stepBack | `useQxListSelection` + `@tanstack/react-virtual` 有界 DOM | `QxModuleSearch` | Rust clipboard DB；**打开端口** `openSession.prefetchClipboardOpen`（只取热 history；idle warm；SWR 先画 store）；**冷存储** `loadMoreClipboardHistory`（列表滚到底 / 键盘近底加载）；**图片端口** `imageAssets.resolveClipboardPreviewAsset` → `clipboard_file_preview` → asset URL | 快捷键打开只预取热窗口，实时内容由原生监听器写库；列表视口串行请求 320px 派生图，详情请求 1600px 派生图，OCR 共用后端解码锁；原图字节不跨 IPC；滚底再拉更早记录；面板非 lazy |
...
| **rss** | feeds / articles / detail | shell 各层；`goBack` 嵌套；Enter 遵循阅读层级 | `useQxListSelection` + `useQxMasterDetail`（文章） | `QxModuleSearch` + `QxListLoading`；输入 160ms debounce，旧查询不回写 | `rss.db` + 30 秒 feeds/article-list SWR + 12 scope LRU + 默认目录 seed + 阅读进度 + 64px 本地图标缓存 + 正文图片缓存 + `rss_meta.last_refresh_all_at` | RSS 打开直接挂载并先画缓存；feeds/folders 并行 single-flight。文章 IPC 默认只取 120 条、摘要最多 2000 字符且不带正文，筛选/搜索在 SQLite 完成；`rss_get_article` 按需取全文，所有列表/详情 DB 读离开 UI 线程。启动后台按 retention/max-per-feed 修复旧库，并用 feed/published 组合索引避免排序全扫。RSS 作为深层内置 React 工作流直接复用 Workbench 的 Shell、selection、master-detail、split、media、reply、search/loading 与 `QxActionSections` 端口；文件夹/日期分组、受信任清洗后的原文 HTML、阅读进度和领域 Dialog 留在 RSS，不绕进插件 iframe RPC。右侧 Context 是唯一完整 Action 面并按领域分组，不再投影 Bottom Bar Actions 菜单或裸键；Enter 依次进入订阅、打开文章、正文返回文章列表并恢复列表焦点，`Cmd/Ctrl+R` 刷新当前订阅，`Cmd/Ctrl+D` 保存文章，`Cmd/Ctrl+S` 下载离线 HTML（图片嵌入 Data URL），`Cmd/Ctrl+Shift+R` 全部刷新。嵌套 leave 已对齐 host Esc；单 Feed 刷新用 activity，刷新全部按 Rust `rss:refresh-progress` 的真实 completed/total 驱动 Island；后台刷新可在 Settings 选择关闭或 6 / 12 / 24 小时，复用同一全量流程与刷新锁，面板未挂载也可运行，手动全量刷新重新计时；favicon 30 天复用并支持 stale fallback；正文远程图片通过 `rss_cache_article_image` 复用宿主代理、逐文件扩展 asset protocol scope 并转成本地 asset URL；Settings → Reader View 图片模式为全宽 / 固定大小 / 小图，小图点击走共享 `QxMediaViewer` |
...
| **macros** | 录制器 + Workbench 宏列表 | `useQxModuleShell`；独立 `MacroCaptureSession` stop port；播放 task stop/join port | `useQxListSelection` + `useQxMasterDetail` + `QxResizableSplit`；步骤详情与 Actions 使用同一 selection/action contract | `QxModuleSearch` | macro store；原生 raw event 有界队列；单实例后台 playback worker；`macro:recording` 坐标事件；每显示器透明点击穿透 cursor overlay；`macro:playback` 真实进度事件；module Island task bridge；`macro_create_demo` smoke macro；Settings → Extensions → Macro Recorder 的尾部裁剪偏好 | 录制的 Esc、按钮、模块卸载、应用退出统一 stop；录制开始即把可停止的 task Island 请求浮出，录制结束按默认 2 秒尾部裁剪；录制时显示可点击穿透的指针与点击环，录制卡片/灵动岛显示坐标；点击步骤保留坐标；播放由 Stop/Esc/应用退出 cancel + join，模块视图卸载保留后台任务并继续显示可停止的灵动岛；播放延迟上限 60 秒、25ms 取消粒度；列表 Enter 进入详情/播放，步骤详情高亮当前步骤；macOS event tap / Windows keyboard+mouse hook 不复用截图指针监听；示范宏用原生地址栏快捷键和 `text_input` 跨平台搜索 `hello` |
| **qx-tty** | 终端 | shell | 侧栏 session 自管 | — | PTY invoke | 可选将来 `useQxSelectableList` |
| **qx-ai** | workbench（左会话列表 + 右自绘聊天）/ settings | shell；settings leave → workbench；Esc leave → launcher | `useQxListSelection` + `useQxMasterDetail` + `QxResizableSplit` | 顶栏搜索过滤会话；`/` Skill；消息队列 | AI store；上次会话；**Capability 目录** `list_qx_capabilities` / `run_qx_capability` + `list_plugins` / `run_plugin_command`；**宿主管理** `list_qx_settings` / `set_qx_setting` + `set_plugin_enabled` / `uninstall_plugin`；Skill frontmatter `capabilities:` 绑定模块/插件；Module Action；`~/.qx/skills` 种子 skill | Skill 驱动执行 Qx 模块与插件；可写设置走 registered adapter，插件管理复用 registry lifecycle 与危险操作确认；插件 `context.ai.actions.*` + launcher command 可扩展；Workbench 右栏自绘对话 |
| **p-zai** | RSS 工作台 + 侧栏对话 | shell；窄屏 master–detail inner Esc | list + article read stack | `QxModuleSearch` | 复用 RSS invoke + QxAI agent harness；`pzai_*` tools + 共享 Module Action 目录 | 对话可 `run_module_action` / `rss_refresh_all` 刷新订阅；不另造 tool 注册路径 |
| **settings** | 设置壳 | shell（query=筛选）；leave=`closeSettings` | — | `QxModuleSearch` | settings store + **`openSettings` returnTo**；Extensions 行内 Toggle 统一写 `builtin_modules` / plugin enabled port | 模块深链进设置后 Esc 回调用方，不再一律 launcher；所有内置模块可直接启停，Clipboard 原生轮询同步遵循 runtime flag |
...
|---------|------------|-----------|----------|------|------|
| **another-boring-piece** | ✅ | ✅ | **host Workbench List + media detail + Actions**；http + system wallpaper/download + 精确文件写入 + clipboard/open-url/notifications | 6 小时 catalog SWR + 宿主 Workbench 图片磁盘缓存 + 20 槽本地壁纸环 + 200 条用户历史 | 今日作品/历史收敛到单面板；自动轮换默认关闭且按 30 分钟 heartbeat 检查用户间隔；声明 `backgroundCategory: wallpaper` 接入宿主跨插件暂停；详情使用稳定 landscape 舞台；保留原作者 `yevgen_glukhov` 与贡献者署名；无 Raycast runtime/shim、自绘 shell/CSS 或无界文件缓存 |
| **agent-usage** | ✅ | ✅ | **host Workbench List + detail + Actions**；`agent.usage` Home Surface Provider；http 二进制通道 + 精确本机登录态只读 invoke + 官方 CLI device-auth + clipboard/open-url | persist SWR（仅归一化配额，不含 token/原始响应） | Dashboard 直接读取安全快照并绘制 Codex/Grok 剩余额度，不启动插件 runtime；设置控制服务商可见性；登录/重新连接由官方 CLI 持有凭据；缺失/过期/网络/解析错误分别呈现，不写回 token、不模拟用量；无 Raycast runtime/shim、自绘 shell 或 CSS |
...
| **v2ex** | ✅ | ✅ | **host Workbench** List + detail + replies + tabs/filters + actions；http + invoke v2ex* | 7 天 retain / 可配 fresh TTL 的 persist SWR + 分 tab/node 宿主快照 + host transport disk | 每次非强制加载先读列表/回复缓存；过期缓存立即呈现并在当前 Workbench 回画重验结果，不能先发布 settled empty 或只更新磁盘；内置面板已移除；Latest/Hot 无需 token，Nodes/Notifications 需要；Enter/Esc 走宿主 |
...
| **speedtest** | ✅ | ✅ | **host Workbench List** + cli/http/system/file + island + clipboard/open-url | persist last result | Raycast Speedtest 原生移植：官方 Ookla CLI 校验安装、JSONL 实时进度、真实采样曲线；保留原作者 tonka3000 与 mcxen；无 SVG 仪表盘或 Raycast shim |
