...
> 状态：Current · 适用版本：v0.6.103 · Owner：Release · 最后复核：2026-08-31
...
- 前端通过 `qx_update_check` 读取可选更新源（设置项 `general.update_source`，**仅在「关于」UI 编辑**；「通用」只保留自动更新开关）。默认 `auto` 会比较 CNB、配置的镜像和 GitHub 的有效清单，选择版本最高者；也可固定为 `cnb` 或 `github`。
