...
| `display.rs` / `display_windows.rs` / `display_macos.m` | Qx 系统级显示器服务：统一枚举、捕获映射与显示器控制；macOS 构建时内嵌 DisplayServices 和 DDC/CI I2C/IOAVService，公共 `display_brightness_*` 覆盖内置屏与外接屏，并返回 DDC 原始值及发现/读写诊断，不启动外部 DDC 可执行文件。 |
