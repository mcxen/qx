...
| `display.rs` / `display_windows.rs` | Qx 系统级显示器服务：统一枚举、稳定 ID、主屏/内置屏/外接屏识别、鼠标所在屏幕、Tauri↔捕获后端映射、区域 still-frame 捕获（`capture_region`）；Windows 以 xcap WGC 为主路径，并由系统服务统一提供 GDI fallback（远程桌面、虚拟显示驱动或 D3D/WGC 不可用时），业务模块不得直接选择后端或重复抓帧；公共 IPC `display_list`（进入截图模块时用于预热短 TTL 原生显示器缓存） |
