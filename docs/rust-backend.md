...
| 文件 | 说明 |
|---|---|
| `g4f.rs` | 内置 OpenRouter（默认）与 DeepSeek BYOK provider + 自定义 OpenAI-compatible BYOK；内置供应商固定 endpoint/model，用户只保存 API Key；`qxai_stream_chat_events` 起线程通过 `qxai-stream` 事件回推 chunk（UTF-8/GB18030 容错 SSE） |
