...
工作流专用：`screencap_select_display(monitor_id)`（保留为旧调用门面，主界面不再暴露）、`screencap_confirm_region_select(...)`、`screencap_set_picker_passthrough`、`screencap_set_pointer_follow(enabled)`、`screencap_toggle_controls` / `screencap_set_controls_pinned`、`start_recording` / `stop_recording` / 历史命令。鼠标跨屏识别仍由根级 `display` 服务完成；区域抓帧底层走 `display::capture_region`；标注合成与历史仍属 screencap。
