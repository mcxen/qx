...
轻量浮窗（label `island`，蓝本 = screencap recording-controls 旗标，**非** main NSPanel）。默认由 `appearance.island_float_enabled=false` 关闭；启用后由 Qx 放在主屏工作区右上角，普通模块/插件状态按 `island_float_rotate_secs` 轮播，重要事件抢占。
