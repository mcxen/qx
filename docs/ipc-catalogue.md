...
> 状态：Current · 适用版本：v0.5.16 · Owner：Backend · 最后复核：2026-07-14
>
> 事实来源：`src-tauri/src/lib.rs` 中的 `tauri::generate_handler!`

Qx 前后端通过 Tauri v2 的 `invoke` 通道通信。当前 `tauri::generate_handler!` 注册 **186 个命令**；不要引用易漂移的固定行号。本文按领域解释主要接口，文末“注册命令基线”必须与注册宏逐项一致。
