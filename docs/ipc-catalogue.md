...
`screencap_begin_capture_select(mode)` 在鼠标所在显示器打开圈选层，`mode` 为 `screenshot` 或 `recording`；圈选会携带稳定显示器 ID，支持内置屏与外接屏。`screencap_confirm_region_select(area, options, action?, annotation_overlay_base64?)` 在用户移动/缩放选区后按最终 action 保存 PNG 或启动 H.264 录制；截图标注以透明 PNG 层传入并由 Rust 合成。`screencap_set_controls_pinned` 控制 340×36 捕获灵动岛是否在空闲时长期外显。
