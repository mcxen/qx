...
> 状态：Current · 适用版本：v0.6.103 · Owner：Backend · 最后复核：2026-08-31
>
...
工作流专用：`screencap_select_display(monitor_id)`（保留为旧调用门面，主界面不再暴露）、`screencap_region_picker_ready()`（WebView 挂载后重放当前 picker session 并重新置前/聚焦）、`screencap_picker_snapshot(monitor_id?)`（返回会话启动时对应显示器的只读快照 `{ monitorId, path, width, height }`，供活动 picker 与外屏 shade 1:1 铺底）、`screencap_selection_preview(area)`（从同一不可变整屏快照裁切当前逻辑选区并编码为轻量 PNG base64，仅供截图马赛克实时预览；不重新抓取桌面、不写历史、不放快门音）、`screencap_confirm_region_select(..., annotations?, captureOptions?, copy_to_clipboard?, dismiss_ui?)`（`annotations` 是选区归一化的截图矢量数组，含 text/arrow/rect/pen/number/mosaic；Rust 在冻结裁切图上先执行马赛克，再按 picker 的 source-pixel scale 用同一合成器绘制全部矢量，不接受浏览器栅格 overlay；录屏不接受任何标注；`captureOptions` 是可选兼容扩展，包含保存/打开、缩略图、选区记忆、指针/点击、麦克风、提示音和 `pinToDesktop` 桌面贴图；`dismiss_ui` 用于 ⌘C/Ctrl+C 复制后保持主界面隐藏）、`screencap_pin_image(path, width?, height?, monitor_id?)` / `screencap_pin_close(label)` / `screencap_pin_close_all()`（Snipaste 风格桌面贴图：always-on-top 无边框窗，label 为 `capture-pin-*`）、`screencap_list_audio_inputs()`（返回稳定 `AudioInput { id, name, isDefault, available }`）、`screencap_recapture_last_region`（无圈选层、按上次逻辑选区静默截图；全局快捷键 `recapture_last_region`）、`screencap_set_picker_passthrough`、`screencap_set_pointer_follow(enabled)`、`screencap_set_picker_interaction_lock(locked)`（拖拽中钉住当前屏，防 Windows 跨屏 handoff 清草稿）、`screencap_toggle_controls` / `screencap_set_controls_pinned`、`start_recording(area?, options?, captureOptions?)` / `stop_recording` / 历史命令。鼠标跨屏识别仍由根级 `display` 服务完成；圈选启动时通过 `display::capture_region` 冻结全拓扑，截图标注合成、快照生命周期与历史仍属 screencap。Windows 截图和录屏共享 WGC 健康策略：远程会话直接使用 GDI；实体机会话若 WGC 返回近全黑空帧，则在截图持久化或录屏编码前拒绝该帧并回退 GDI，录屏时间轴从首个有效 fallback 帧开始。
...
`screencap_confirm_region_select`, `screencap_selection_preview`, `screencap_picker_snapshot`, `screencap_recapture_last_region`, `screencap_region_select_status`,
