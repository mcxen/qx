...
AI Agent 的默认模型置于供应商列表之前；工具开关、原生 Tool Calling 与迭代上限归入
工具与安全。供应商列表仅用线性分隔，新增/编辑复用宿主 Dialog，关闭后恢复触发按钮焦点
和原列表滚动位置。内置模板只展示接口地址并编辑 API Key；自定义供应商保留完整字段和
获取模型操作。模型表格按实际容器宽度折行，不依赖整个窗口的宽度。
设置导航默认 220px，用户手动调整的 Context 宽度继续生效。
