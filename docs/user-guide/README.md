...
> **版本**：v0.6.26 · **适用平台**：macOS 14+ · Windows 10/11

**Qx** 是一款后台常驻的效率启动器，灵感来自 [Raycast](https://raycast.com)。按全局快捷键唤起，搜索应用、文件、剪贴板历史和插件，执行后再次隐藏。
...
在搜索框输入关键词，Qx 会同时搜索应用、文件、命令和内置模块：
...
- **模块结果**：剪贴板历史、RSS 订阅、AI 会话等（可在搜索设置中开关）
...
顶部有 **"启用模块搜索"** 总开关。关闭后，内置模块不再为主启动器提供搜索结果。
...
> 关闭某个模块的搜索不影响在模块内部浏览和使用，仅影响主启动器的搜索结果。
