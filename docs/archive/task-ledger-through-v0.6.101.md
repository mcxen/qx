# Qx 历史任务台账（截至 v0.6.101）

> **历史归档，不是当前待办。** 此文件保留 v0.6.101 及更早版本的完成记录、旧验证项和当时的实现计划；其中未勾选项不会自动继承为当前承诺。当前工作只看仓库根目录的 [`TASK.md`](../../TASK.md)。
>
> Settings/About 面板的结构、设计令牌、Row/Card 规范与响应式断点见 [`docs/settings-panel.md`](../settings-panel.md)。

## Fix — Home Dashboard 小窗口组件边界

**状态**：布局修复、自动验证、本地浏览器与签名桌面运行态复核完成。

- 修复 `640px` 容器断点被 `.has-pinned` 宽屏规则以更高选择器优先级覆盖的问题；窄容器现在
  确定切为“置顶入口 → 双列指标”，不再把两组组件强行并排压缩。
- 宽屏紧凑等分时，置顶网格和指标列各自在自身边界内滚动；存在 RSS / Agent Usage 等整行
  卡片或进入窄断点时，主区按真实内容增高并由 Dashboard 纵向滚动。
- 相邻卡片保持边界分离、无横向溢出，置顶应用仍保留原有排列顺序。

### 验证

- [x] `npx tsc --noEmit` / `npm run check` / `npm run build`
- [x] 本地浏览器 10 个置顶入口：980×576 宽断点、620×500 窄断点及默认 RSS 整行卡片组合
  均保持组件边界分离、无横向溢出；窄断点为单列主区，扩展组合由 Dashboard 纵向滚动。
- [x] `npm run tauri build` 后使用本机 Apple Development 身份签名并安装至
  `/Applications/Qx.app`；真实原生 WebView 小窗口中置顶入口、指标与最近搜索边界分离，
  运行进程路径和 0.6.101 包版本核对通过（2026-08-30）。旧 `~/.qx/logs/qx.log` 未追加本次
  启动行，因此该旧日志不作为本次桌面验收依据。

## Fix — Workbench 文章属性统一进入副标题

**状态**：宿主呈现、规范、门禁与本地可视回归已完成；等待签名桌面 QxWeibo 复核。

- 含正文、原位内容或回复的资讯/社区 Detail 统一把顶层 `fields[]` 投影到标题下方元数据区，
  支持按详情栏宽度自然换行；微博等插件不再在正文后重复显示整张文章属性表。
- 宿主按副标题分段去除已存在的作者、发布时间等同值项，同时保留点赞、评论、转发、来源、
  地区等不同字段；结构化数据本身不删除，离线 HTML 与缓存契约保持不变。
- 管理型 Workbench 顶层字段和 `sections[].fields[]` 仍使用原字段表，避免影响 Sysinfo、
  表单、任务历史等非文章详情。

### 验证

- [x] 文章字段提升、作者/时间去重、零值保留、管理字段不提升专项断言
- [x] `npx tsc --noEmit` / `npm run check` / `npm run build`
- [x] 本地真实 React Workbench 夹具：360px 视口副标题分为两行、无横向溢出且字段表数量为 0
- [ ] 桌面态：QxWeibo 宽/窄详情栏属性换行且正文下方无重复表；管理型 Workbench 字段表不变。

## Fix — 截图全屏蒙版与标注稳定性

**状态**：snapshot-first 重构与专项回归进行中；待 `imsalt` 本地签名包桌面交互复核。

- 圈选入口先隐藏应排除的 Qx 界面并冻结全部显示器 framebuffer，再显示最顶层 picker/shade；
  活动屏与外屏都铺设各自的 1:1 静态底图，最终 PNG、马赛克预览与标注统一裁切同一份像素，
  不再在确认时二次抓取实时桌面。会话结束后删除包含整屏内容的临时快照。
- 录屏模式删除马赛克：不显示工具栏入口、不响应数字标注快捷键、不传录屏遮挡参数，Rust
  录制引擎也不再为每一帧复制并执行像素化；截图模式的马赛克保持不变。
- macOS picker 与多屏 shade 提升到 Screen Saver 窗口层级，完整覆盖菜单栏、状态项与 Dock，
  仅由原生 NSWindow level 管理且不再被 Tauri `always_on_top` 重置；圈选和标注期间拦截底层
  指针，倒计时与录制边框的既有穿透协议保持不变。
- 静态桌面、选区和标注使用同一选区归一化坐标；蓝色选区框使用 outline，不参与 canvas
  盒模型。确认时前端只发送矩形、箭头、画笔、文字、序号和马赛克矢量，Rust 单一合成器按
  冻结源图像像素比例输出，不再生成或缩放浏览器透明标注层。
- 箭头、矩形、画笔和马赛克改用 Pointer Capture；拖动草稿以同步 ref 保存，动画帧合并预览，
  快速松手直接读取最终草稿，不再依赖尚未提交的 React state。
- 修复文字新建点击被 document capture 监听器立即结束的竞态；结束旧文本的延迟回调不会清掉
  同一次点击新建的文本框。
- 圈选提示按八个屏幕边缘候选位自动避开选区，没有无覆盖位置时隐藏；反馈覆盖层从旧 picker
  文件拆出，`RegionPickerWindow.tsx` 回到 1000 行上限内。

### 验证

- [x] `npx tsc --noEmit`
- [x] `node scripts/check-qx-shell-navigation.mjs`
- [x] `cargo fmt --check` / `cargo check` / snapshot crop与统一合成器单测
- [x] 本地 `imsalt` 签名安装包：冻结帧一致性、菜单栏/Dock 蒙版与点击拦截、顶部圈选成品、连续快速箭头、文字输入、提示避让（2026-08-24 用户实机复核）
...
- File Actions 右上角可切换最近文件历史；记录最近选择或预览过的 5 个文件，按查看时间倒序且按路径去重，可清空。它不是重命名、压缩或解压操作日志，旧操作历史会被丢弃。
...
- 修复 File Actions 未进入 Launcher 可置顶/主页入口目录，以及 Rust `navigate` 已发出但 App 原生事件白名单漏接 `file-actions` 导致 `Option+F` / `Alt+F` 显示窗口却不进入模块的问题。
- 新增独立 QxPreview：在 File Actions 扩展设置中可录制 `Option+O` / `Alt+O` 建议键，直接预览 Finder / Explorer 当前选择；复用 revision 快照和跨平台全局快捷键端口，多选可用上下键切换，Space/再次按键关闭。
- PPTX 预览根据分栏可用宽度动态缩放，并监听窗口/分栏尺寸变化重新适宽。
...
- [ ] macOS/Windows 安装态：置顶 File Actions 后主页立即出现；`Option+F` / `Alt+F` 打开 File Actions；启用 QxPreview 建议键后可直接预览多选文件并用上下键切换。
...
- [x] 社区市场 `main` 与 `v1.7.1` 指向 `9c258fc`；远端索引、包内 manifest、归档大小及 SHA-256 已复核
...
- macOS 正文和封面恢复由 WebView 直接加载原始 HTTP(S) 图片，不经过透明占位或 Rust
  本地资源回填，避免缓存链路失败后只剩大块空白。
- Windows 正文与封面共享 Rust 图片缓存请求和 WebView 本地解码 Promise；列表选中项、
  前后相邻文章及阅读队列下一篇会有界预热首批图片，同一 URL 不重复下载或解码。
...
- [x] `npm run check`
- [x] `npm run build`
- [x] macOS 运行态验证正文图片由 WebView 直接显示，不存在透明占位空白。
- [ ] Windows 运行态验证冷缓存、热缓存及相邻文章预热。
...
- 中文 IME 合成期间保留原生候选窗口并显示拼音 preedit；preedit 只参与文字框实时测量，
  不写入标注状态，候选确认后一次性提交汉字并按右边界重新排版。
...
- 上下左右边只调整对应方向尺寸；四角统一以对角为锚点等比缩放文字框与字号，不再按拖动
  角度切换成只改宽度。
- 输入内容优先单行向右增长，到截图右边界后固定宽度、自动换行并增加高度；删除内容时
  文字框按剩余内容缩回，接近底边时向上调整，避免隐藏文字。
...
- 录屏开始后继续以实际录制选区作为蓝色边框范围；选区内保持原亮度，选区外四周保留
  半透明灰色遮罩，遮罩与边框由受内容保护的 picker 承载，不进入录屏成品。

### 验证

- [x] `npx tsc --noEmit`
- [x] `npm run check`
- [ ] macOS 运行态验证录制中选区内清晰、选区外压暗且成品不含遮罩。
...
- 胶片横向滚动恢复全局浮层细拇指：轨道不再常驻占满详情宽度，滚动提示贴近图片底边，
  停止滚动后自动淡出。
- 全尺寸图片预览修复普通滚轮缩放，并按横图、竖图和长截图的基准尺寸正确放大/缩小；
  放大后可滚动查看完整图片。左右箭头取消按压位移，缩放比例移到右下角、序号移到左下角。
- 修正 WebView 会丢弃 `calc(100% * number)` 的实际缩放根因，改为直接发布标准百分比
  尺寸；滚轮监听提升到整个预览舞台。左右箭头默认隐藏，仅在鼠标进入对应边缘感应区或
  键盘聚焦时显示，触屏等无 hover 环境保持可见。
...
- v0.6.1 已证明上述 esbuild 端口在 Windows 正常工作；随后 module-port 门禁暴露第二个
...
### 截图/录屏双态控制栏与反馈（2026-07-26）

- 截图/录屏入口直接进入拖拽圈选，不显示前置模式条；选区建立后显示截图或录屏控制栏，首按钮切换意图，所有图标提供 Tooltip，工具条按下方、上方、屏内回退并夹紧当前显示器。
- 截图工具提供矩形、箭头、画笔、文字、序号、马赛克、撤销/重做和颜色；区域/全屏按钮可往返恢复。录屏不提供任何标注或马赛克工具，截图标注切录屏前确认并清栈。
- `⌘C / Ctrl+C` 跳过延迟并复制当前截图；内部 PNG 成功后由单一收尾路径播放一次自有快门 WAV。失败截图、录屏和关闭提示音不播放。
- 成品先写 Qx 图库，再导出桌面、文稿、自选目录或剪贴板；支持 0/5/10 秒、浮动完成提示、记忆选区、完成后打开、截图/录屏指针和录屏点击效果。旧 3 秒值兼容映射为 5 秒。
- 新增稳定 `CaptureExecutionOptions` / `AudioInput`；麦克风由按目标架构生成的 `qx-ffmpeg` sidecar 采集 AAC 并合并，失败保留无声视频。sidecar、SHA256、GPL 许可证和自有 WAV 由构建准备脚本生成，不提交架构二进制。
- 宏录制与捕获点击效果共用根级输入事件服务；macOS 增加麦克风用途说明与音频输入 entitlement，Windows 快门音走 WinMM。

#### 验证

- [x] `npx tsc --noEmit`
- [x] `cargo check`（macOS）
- [x] `npm run build` / `npm run check` / `cargo test --lib screencap::` / macOS `.app` bundle 内容与 ad-hoc 深签验证
- [ ] Windows Compatibility 与双平台麦克风、指针、边缘定位手动验证
...
- 捕获预览历史按截图、录屏拆为两个默认展开且可独立折叠的分组，列表/图库布局与键盘导航均只作用于展开组中的可见文件。
- 圈选控制栏固定为单排；全屏模式默认位于屏内中下侧，可拖动并夹紧当前显示器边缘，继续由受保护 picker 排除在截图/录屏成品之外。
- 修复全屏取消后复用 picker 仍停在全屏模式、导致下一次无法拖拽建框；新会话统一重置为区域模式。白色序号标注改用黑色数字与深色轮廓，并同步工具栏预览。
- 圈选改为两阶段确认：首次框选后可移动、四角缩放，再选择截图或录制；截图可添加文字和箭头。旧版浏览器透明标注层已由 2026-08-24 的统一 Rust 源像素矢量合成器取代。
