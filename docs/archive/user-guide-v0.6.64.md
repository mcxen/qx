# Qx 用户指南（v0.6.64 历史版本）

> **历史归档。** 本文只记录 v0.6.64 当时的界面和操作，不应用来判断当前功能、快捷键或存储目录。当前指南见 [`docs/user-guide/README.md`](../user-guide/README.md)。
...
可在主页「编辑快速入口」中增删排序。托盘菜单与主页快速入口相互独立，只显示「设置 → 通用 → 托盘菜单」中加入的项目。
...
- **通用**：开机自启、语言、**自动更新开关**（默认开启；后台更新 Qx，并同步升级当前版本兼容的已安装社区插件）
