...
2. 在 Qx 宿主内通过插件最终使用的 `context.*` 端口逐条实测外部接口主路径；不得用
   浏览器、curl、Node、Python/MCP 或 mock 结果替代。详细门禁见
   [`plugin-development-guide.md`](./plugin-development-guide.md#8-真实接口验收上架门禁)。
3. 记录插件/Qx 版本、平台、时间、脱敏样本、成功/失败路径和已知限制。
4. 生成包并计算仓库要求的摘要或签名。
5. 把不可变版本包上传到插件仓库。
6. 更新 `index.json` 的版本、兼容范围和下载地址。
7. 提交 PR，附真实接口验收记录、跨平台与权限说明。
8. 合并后从真实市场源安装一次，并复测至少一条核心线上路径。
...
- [ ] platforms 无重复且真实验证。
- [ ] 权限最小，快捷键默认关闭。
- [ ] 最终包已在 Qx 宿主内实测每条外部接口主路径；fixture/mock 仅用于回归测试。
- [ ] HTTP 已验证真实成功与错误响应；二进制协议额外验证压缩、Content-Type/Encoding 和前导字节。
- [ ] 验收记录包含版本、平台、时间、脱敏样本和结果，不含 Cookie、Token 或 API Key。
- [ ] 包内无密钥、缓存和临时文件。
- [ ] 索引下载地址指向不可变版本包。
- [ ] 市场源与镜像都执行相同校验。
