...
宿主统一执行 Workbench Action 的按钮、Context、菜单和快捷键入口，处理禁用、防连击和失败反馈。现有 `id/command/kbd` 等序列化字段保持兼容，插件无需同步升级；异步 command 必须返回 Promise。错误通过宿主 Island 单行入口展开完整详情，插件不得在表单、列表或正文下追加错误条；窄窗也保留可访问的错误入口。
