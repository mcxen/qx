...
`.qx-plugin` 冷安装到声明兼容的 Qx 版本，并通过插件实际使用的 `context.*` 端口完成
至少一次真实端到端请求。未完成真实接口验收的版本不得进入市场索引。

- 浏览器、Node `fetch`、curl、Postman、Python/MCP 的成功结果只能用于诊断，不能代替
  Qx 宿主内验收。代理、TLS、重定向、请求头限制和内容自动解压行为可能与宿主不同。
- mock、录制响应和合成 fixture 只证明解析器回归，不证明线上接口可用。真实验收必须覆盖
...
- 每条主路径至少验证一个成功样本，并验证可实际触发的失败形态，例如非 2xx、超时、空体、
  ...
  解析器后显示误导性的协议错误。
- 二进制接口必须用 `bodyBase64` / `arrayBuffer()` 走完整宿主链路，并检查状态码、最终 URL、
  `Content-Type`、`Content-Encoding` 和响应前导字节。应覆盖服务端真实可能返回的压缩与未压缩
  数据，以及 HTML/JSON 错误体；不要假设浏览器的自动解压行为也存在于 Qx HTTP 端口。
- 验收记录至少包含插件版本、Qx 版本、平台、测试时间、脱敏样本标识、各主路径结果和已知限制；
  禁止记录 Cookie、Token、API Key 或完整私人内容。

如果上游接口在发布窗口内不可访问、需要尚未具备的账号/设备，或无法从 Qx 宿主完成真实请求，
该版本应标记为阻塞并推迟上架，不能用 mock 通过来替代。
...
- [ ] 网络、CLI、下载和进度都是真实结果。
- [ ] 最终安装包已在 Qx 宿主内逐条实测外部接口主路径；浏览器/curl/mock 未被当作上架依据。
- [ ] 二进制接口已验证压缩、响应类型与 HTML/JSON 错误体，不会把错误响应误报为协议解析失败。
- [ ] 验收记录包含版本、平台、时间、脱敏样本和结果，且不含密钥或私人数据。
- [ ] `panel.render()` 不等待长任务。
- [ ] 用户可从错误状态重试。
