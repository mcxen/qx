...
Workbench 条目可带：`icon` · `image` · `badge` · `tone` · **`progress`（0–100）** · `status` · `detail` · `actions` · `raw`。List 会把 `item.image` 渲染为左侧缩略图；`detail.image` 可在右侧结构化详情顶部显示自适应大图，并声明 `aspectRatio: "auto" | "landscape" | "square" | "portrait"`、`zoomable`、`caption`；宿主统一处理加载失败与全尺寸预览。`item.status` / `detail.status` 表达不清空旧内容的局部异步状态。`detail.form.controls` 可声明宿主渲染的 `text` / `number` / `select` 受控表单，并通过 `onInput(id, value, item)` 回传变更。`detail` 必须是结构化数据；Workbench 不接受 HTML。
