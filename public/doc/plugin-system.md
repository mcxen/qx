...
| `context.ui.mountWorkbench(state, handlers)` | 声明式受控 panel：Qx 渲染 tabs、稳定的 List/Gallery 空态画布、自适应/可放大的图片详情、局部 async status、text/number/select 表单、Actions 与 island；返回 controller 的 `update/updateItems` 以稳定 id 发布增量/批量业务结果；表单通过 `onInput(id, value, item)` 回传纯数据；宿主即时处理 query/tab/selection，manifest command 完成后回调 `onCommandComplete`；`backgroundPoll` 可绑定后台 interval command |
